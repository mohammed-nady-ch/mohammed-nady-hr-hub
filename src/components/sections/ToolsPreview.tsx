...
export default function ToolsPreview({ lang, setPage }: { lang: Lang; setPage: (page: Page) => void }) {
  ...
  return (
    <section className="section" id="tools">
      <div className="title">
        <h2>{t.featured}</h2>
        <p>{lang === 'ar' ? 'أدوات عملية مبنية حول قرارات الموارد البشرية اليومية.' : 'Practical tools built around everyday HR decisions.'}</p>
      </div>
      <div className="tools">
        {toolCards[lang].map((tool) => (
          <article key={tool.title}>
            ...
            {tool.page ? (
              <button onClick={() => setPage(tool.page!)} type="button">{tool.action} {lang === 'ar' ? '←' : '→'}</button>
